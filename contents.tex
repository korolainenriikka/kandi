...
\section{Timeliness of re-training: concept drift detection and model monitoring}

% TODO: model monitoring. state that this needs to be done & say which metrics should be preferred?

% maybe add also monitoring to this subheading & content, model should also be monitored and updated based on that
...
% add: statistical tests are an indirect indication, whereas feedback is a direct indication. therefore monitoring model feedback should also be in place because that would then notice faults in the drift detector: does model accuracy correlate with the detected drifts or not? rewrite this: monitoring is needed. Add concluding sentence: data monitoring and model metrics monitoring should notice all drift types.
